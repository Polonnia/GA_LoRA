...
from fisher_sam import run_lora_fisher_sam
from focal_sam import run_lora_focal_sam
...
def main():
    ...
    #results/shots16/imagenet-sketch dataset = build_dataset(args.dataset, args.root_path, 0, preprocess)
    ...
    for s in shots_list:
        ...
        if base_result_path is not None:
            result_root_with_backbone = os.path.join(base_result_path, backbone_tag, rank_tag)
            if s == -1:
                args.result_path = os.path.join(result_root_with_backbone, "shots_all", args.dataset)
            else:
                args.result_path = os.path.join(result_root_with_backbone, f"shots{s}", args.dataset)
        ...
        if args.eval_only:
            if args.shots == 0:
                ...
                target_device = torch.device(f"cuda:{args.gpu_ids[1]}") 
        ...
        if args.opt == 'adam':
            ...
            run_lora_adam(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0], train_from_ga=args.train_from_ga, finetune_full=False)
        elif args.opt == 'sgd':
            print("Running LoRA with SGD optimization")
            run_lora_sgd(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0], momentum=False)
        elif args.opt == 'sgd_momentum':
            print("Running LoRA with SGD with momentum optimization")
            run_lora_sgd(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0], momentum=True)
        elif args.opt == 'entropy_sgd':
            print("Running LoRA with Entropy-SGD optimization")
            run_lora_entropy_sgd(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0])
        elif args.opt == 'ga':
            ...
            run_lora_sam(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0])
        elif args.opt in ('fisher_sam', 'fsam', 'fishersam'):
            print("Running LoRA with Fisher-SAM optimization")
            run_lora_fisher_sam(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0])
        elif args.opt == 'focal_sam':
            print("Running LoRA with Focal-SAM optimization")
            run_lora_focal_sam(args, clip_model, logit_scale, dataset, device_id=args.gpu_ids[0])
        else:
